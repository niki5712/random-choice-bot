...
class TelegramAPI:
    ...
    def api_call(
        ...
        for n in range(1, 1 + retries + 1):
            try:
                start = perf_counter()
                previous_start = self._method_to_previous_start_map.get(method, start - timeout)

                time_to_wait = timeout - (start - previous_start)
                if time_to_wait > 0:
                    if method != 'getUpdates':
                        self._logger.debug(f'sleep({time_to_wait}) for timeout')
                    sleep(time_to_wait)

                self._method_to_previous_start_map[method] = perf_counter()

                self._logger.debug(f'Call {self}.api_call({method!r}, {parameters!r})')
                response = self.perform_request(method='post', path=method, json=parameters)
                # response.raise_for_status()  # TODO: проверить не лишаюсь ли я информации об ошибке из-за этой строки
